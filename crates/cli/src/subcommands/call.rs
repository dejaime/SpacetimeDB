...
impl<'a> CallResult<'a> {
    fn params(&self) -> &'a sats::ProductType {
        match self {
            CallResult::Reducer(reducer_def) => &reducer_def.params,
            CallResult::Procedure(procedure_def) => &procedure_def.params,
        }
    }
    fn name(&self) -> &str {
        match self {
            CallResult::Reducer(reducer_def) => &reducer_def.name,
            CallResult::Procedure(procedure_def) => &procedure_def.name,
        }
    }
    fn kind(&self) -> &str {
        match self {
            CallResult::Reducer(_) => "reducer",
            CallResult::Procedure(_) => "procedure",
        }
    }
}
...
pub async fn exec(config: Config, args: &ArgMatches) -> Result<(), Error> {
    ...
    let call_result = match module_def.reducer(&**reducer_procedure_name) {
        Some(reducer_def) => CallResult::Reducer(reducer_def),
        None => match module_def.procedure(&**reducer_procedure_name) {
            Some(procedure_def) => CallResult::Procedure(procedure_def),
            None => {
    ...
    let arguments = arguments
        .unwrap_or_default()
        .zip(&call_result.params().elements)
    ...
    if let Err(e) = res.error_for_status_ref() {
        ...
        let error_msg =
            if response_text.starts_with("no such reducer") || response_text.starts_with("no such procedure") {
                ...
                invalid_arguments(&database_identity, database, &response_text, &module_def, call_result)
            } else {
                return error;
            };
    ...
    if let CallResult::Procedure(_) = call_result {
...
fn invalid_arguments(
    ...
    write!(
        ...
        call_result.kind(),
        ReducerSignature(module_def.typespace().with_type(&call_result))
    )
    .unwrap();
...
struct ReducerSignature<'a>(sats::WithTypespace<'a, CallResult<'a>>);
...
fn no_such_reducer_or_procedure(
    ...
    let mut error = format!(
        "No such reducer OR procedure `{reducer}` for database `{db}` resolving to identity `{database_identity}`."
    );

    add_reducer_procedure_ctx_to_err(&mut error, module_def, reducer);
...
fn add_reducer_procedure_ctx_to_err(error: &mut String, module_def: &ModuleDef, reducer_name: &str) {
    ...
    if let Some(best) = find_best_match_for_name(&reducers, reducer_name, None) {
        ...
        let mut list_similar = |mut list: Vec<&str>, name: &str, kind: &str| {
            ...
            let too_many_to_show = list.len() > REDUCER_PRINT_LIMIT;
            let diff = list.len().abs_diff(REDUCER_PRINT_LIMIT);
            list.truncate(REDUCER_PRINT_LIMIT);
